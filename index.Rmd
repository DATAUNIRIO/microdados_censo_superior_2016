...
par(mar = c(0.5, 8.5, 0.5, 0.5), mgp = c(5, 1, 0))
...
tabela1<-table(ALUNO_UNIRIO16$CO_UF_NASCIMENTO)
tabela1<-data.frame(tabela1)
invisible(Mapa_basededados <- merge(Mapa_brasil, tabela1, by.x = "State", by.y = "Var1", all = TRUE))
```

```{r, eval=FALSE}
library(tmap)
tm_shape(Mapa_basededados) + tm_fill("Freq", style="jenks") +
...
Existem diversas outras análises possíveis. 
